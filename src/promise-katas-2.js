...
const cat = () => {
    return fetch("cats").then(() => {
        return ["Bandit", "Berry", "Puss in boots", "Smokey"];
    })
};
...
const joke = () => {
    ...
    const promise1 = fetch("jokes", "question").then(() => {
        newJoke.question = "Why did the scarecrow win the Nobel Prize?";
    });
    const promise2 = fetch("jokes", "answer").then(() => {
        newJoke.answer = "Because he was out-standing in his field.";
    });
